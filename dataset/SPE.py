...
class SPE(Dataset):
    def __init__(self,  root_dir='SPE', subset='train', categories: list = None, kp_num = 8, img_size=512, nshot = 10,
        ...
        self.support_base_dir = os.path.join(self.root_dir, 'support')
        ...
        if not os.path.exists(self.subset_sequences_file):
            ...
            for category in categories:
                assert category in self.subset_sequences, 'Category {} is not in {} subset!'.format(category, self.subset)
    ...
    def __getitem__(self, idx):
        ...
        # adjust adj matrix of presentation
        adj_pres = self.skeleton2adj(skeleton_pres)

        if gt_kp_num < self.kp_num:
            size = adj_pres.size(0)
            adj_pres_expend = torch.zeros((self.kp_num, self.kp_num), dtype=adj_pres.dtype)
            adj_pres_expend[:size, :size] = adj_pres

            adj_pres = adj_pres_expend
        ...
        pool = self.support_pool[category]
        pool_size = pool['images'].size(0)

        sel_idx = random.sample(range(pool_size), self.nshot)

        sup_image = pool['images'][sel_idx]
        kp_2d_sup = pool['kps_2d'][sel_idx]

        mask_s = torch.zeros(self.kp_num)
        mask_s[:gt_kp_num] = 1
        mask_s = mask_s.unsqueeze(-1)

        # now mask_s from dataloader shape is [bs, kp_num]
        return img, np.asarray(pos), np.asarray(orient), kp_2d, kp_body, adj, gt_kp_num, bbox, sup_image, kp_2d_sup, mask_s, adj_pres

    def _preload_sup(self):
        print(f"==> Preloading support data into memory for {self.subset}...")

        cat_dirs = [d for d in os.listdir(self.support_base_dir)]

        for category in cat_dirs:

            annos_sup = pd.read_csv(os.path.join(self.support_base_dir, category, 'annotations.csv'))
            kp_body_sup = np.loadtxt(os.path.join(self.support_base_dir, category, 'keypoints.txt'), delimiter=',')
            gt_kp_num = kp_body_sup.shape[0]

            images_sup = []
            kps_2d_sup = []

            for i in range(len(annos_sup)):
                pos_sup = eval(annos_sup['position'].iloc[i])
                orient_sup = eval(annos_sup['orientation'].iloc[i])
                orient_mat = self.transform_mat.T @ self.quaternion2matrix(orient_sup) @ self.transform_mat
                orient_sup = self.matrix2quaternion(orient_mat)
                pos_sup = pos_sup @ self.transform_mat

                img_path = os.path.join(self.support_base_dir, category, 'image', f'{i + 1:06d}.jpg')
                img_sup = Image.open(img_path)

                kp_body_transformed = kp_body_sup @ self.transform_mat
                _, kp_2d_sup = self.keypoints(None, kp_body_transformed, pos_sup, orient_sup, self.camera_matrix)

                kp_2d_sup = [kp for kp in kp_2d_sup if
                             (0 <= kp[0] < self.org_img_size) and (0 <= kp[1] < self.org_img_size)]
                kp_2d_sup = np.asarray(kp_2d_sup)

                if kp_2d_sup.shape[0] < gt_kp_num:
                    continue

                if self.transformation:
                    img_sup, kp_2d_sup, _ = self.transform(img_sup, self.img_size, kp_2d_sup)

                if kp_2d_sup.shape[0] < self.kp_num:
                    num_to_add = self.kp_num - kp_2d_sup.shape[0]
                    padding = np.tile(kp_2d_sup[0], (num_to_add, 1))
                    kp_2d_sup = np.vstack([kp_2d_sup, padding])
                else:
                    kp_2d_sup = kp_2d_sup[:self.kp_num]

                images_sup.append(img_sup if isinstance(img_sup, torch.Tensor) else F.to_tensor(img_sup))
                kps_2d_sup.append(torch.as_tensor(kp_2d_sup, dtype=torch.float32))

            self.support_pool[category] = {
                'images': torch.stack(images_sup),
                'kps_2d': torch.stack(kps_2d_sup),
                'gt_kp_num': gt_kp_num
            }



    def transform(self, img: Image.Image, out_size: int, kp_2d: np.ndarray) -> (torch.Tensor, np.ndarray, np.ndarray):
        img = F.pil_to_tensor(img)

        raw_bbox = self.get_bbox(kp_2d)
        bbox = [int(round(x)) for x in raw_bbox]
        ...
        padding_bottom = roi_size - bbox[2] - padding_top
        padding_right = roi_size - bbox[3] - padding_left
        ...
        scale_factor = out_size / roi_size
        kp_2d = np.float32(kp_2d * scale_factor)

        return img.float(), kp_2d, np.array(bbox)
    ...
    def keypoints(
        ...
        keypoints_uv = np.vstack([
            keypoints_pixel[:, 0] / keypoints_pixel[:, -1], keypoints_pixel[:, 1] / keypoints_pixel[:, -1]]
        ).transpose().astype(np.float32)  # Nx2
    ...
    def metric(
        ...
        pos_p = pos_p.cpu().numpy() if isinstance(pos_p, torch.Tensor) else pos_p
        pos_gt = pos_gt.cpu().numpy() if isinstance(pos_gt, torch.Tensor) else pos_gt
    ...
    def PnP(kp_2d, kp_3d, camera_matrix):
        ...
        _, rvec, tvec = cv2.solvePnP(kp_3d, kp_2d, camera_matrix, distCoeffs=None, flags=cv2.SOLVEPNP_EPNP)
        rotation_matrix, _ = cv2.Rodrigues(rvec)
        quaternion = SPE.matrix2quaternion(rotation_matrix)
